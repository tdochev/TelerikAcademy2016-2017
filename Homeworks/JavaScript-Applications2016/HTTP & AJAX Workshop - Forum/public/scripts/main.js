...
$(() => { // on document ready
    ...
    function loadThreadsContent(threads) {
        ...
        threads.forEach((th) => {
            let currentThreadUI = getThreadUI(th.title, th.id, th.username, new Date(th.postDate).toLocaleString());
    ...
    function loadMessagesContent(data) {
        ...
        if (data.result.messages && data.result.messages.length > 0) {
            data.result.messages.forEach((msg) => {
                messagesContainer.append(getMsgUI(msg.content, msg.username, new Date(msg.postDate).toLocaleString()));
            });
        } else {
            messagesContainer.append(getMsgUI('No messages!', data.result.username, new Date(data.result.postDate).toLocaleString()));
        }
